...
module.exports = function (app, db) {
    ...
    function getMains(name, callback) {
        let sql = `SELECT Main, Title, Name, Type
                    FROM Content
                    WHERE Name = ? `
    ...
    //del 시작
    app.delete('/api/content/manage/:Name/:num', function(req, res) { 
        ...
        try {
            ...
            let sql = `DELETE From Content WHERE Name = ? and No=? `

            if (Name === undefined || Name.length <= 0 ||
                Num === undefined || Num.length <= 0 ) {
                JsonResponse.sendJsonResponse(res, 400)
                return
            }

            getContent(Num, (err, row) => {
                console.log(Name)
                if (err) {
                    console.error(err.message)
                    JsonResponse.sendJsonResponse(res, 404)        
