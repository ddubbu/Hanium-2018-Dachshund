app.controller("ContentController",
     ...
     */
            ...
            $scope.forcheck = function () {
                ...
                if (count == 0) {
                    ...
                    if ($scope.checkedNo != 0) {
                        ...
                        $scope.modifypage()//$scope.checkedNo)
                    }
                }
            ...
            $scope.modifypage = function () {
                console.log('modifypage 넘어옴')
            ...
            $scope.togoModify = function () {
                let data = {
                   // no: $routeParams.Num,//$scope.inputData.No,
                    b_sel: $scope.inputData.B_sel,
                    t_sel: $scope.inputData.T_sel,
                    s_date: $scope.inputData.start_date,
                    e_date: $scope.inputData.end_date,
                ...
                console.log('우선 data',data)
                

            RequestFactory.put('/api/content/modify/' + $routeParams.Num,data)//+ <--여기 수정함!
                .then(function onSuccess(response) {
                    if (response.status === 200) {
                        alert('수정완료');
                        $location.path('/managecontent')
