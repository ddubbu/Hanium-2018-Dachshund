...
module.exports = function(app, db) {
        ...
        if(Board.IsApprove === 1){
            ...
            try{
                ...
                allApprove(     (err,row) => {
                    ...
                    if (row) {
                        console.log(row)
                        db.run(sql, [Board.IsApproval], (err) => { //정의한 sql 순서대로
                            if (err) {
                                console.error(err.message)
                                JsonResponse.sendJsonResponse(res, 404)
    ...
    //get ; content_all 
    app.get('/api/content/all', function() {
        //post와 put을 제외한 작업엔 무필요 let location = req.body
        //body에 data가 없으니.
        //let Name=req.params.Name //간략화
        console.log('app.get 시작')
        //console.log(Name)
        try {
            //변수 없. 아무것도 안 넘김.
            getAllBoard(     (err, row) => {
...
/*
    //get ; content_all 
    app.get('/api/content/all/:Name', function(req, res) {
        //post와 put을 제외한 작업엔 무필요 let location = req.body
        //body에 data가 없으니.
        let Name=req.params.Name //간략화
        console.log('app.get 시작')
