app.controller("BoardController",
               ['$scope', 
                '$rootScope',
                '$location', 
                '$window', 
                '$routeParams',
                'UserAuthFactory',
                'AuthenticationFactory',
                'RequestFactory',
    function($scope, $rootScope, $location, $window, $routeParams,
             UserAuthFactory, AuthenticationFactory,RequestFactory) {

        $scope.isLogin = false
        $scope.items = [ ]

        //컨텐츠 관리 버튼 이동 솔루션
        //<이동 이분화>-->굳이?
        $scope.BoardName = '';
        $scope.aaa = '';


        
        //모아보기 클릭시
        $scope.togo = function(name){
            console.log(name)
            $scope.BoardName = name
            $scope.aaa =name
            //console.log(p)
           // var BoardName= name
        }
        $scope.togo_all = function(){
            //$scope.init_Main2()
           // console.log(items)
           // $location.path('/content_text')

            console.log("togo_all")
            //$scope.init_Main2() //$scope.delName $scope.BoardName = name
            $location.path('/content_text')
  /*          RequestFactory.get('/api/content/manage/'+ $scope.BoardName)
               .then(function onSuccess(response) {
                       if (response.status === 200) {
                           
                           console.log('요놈')
                           console.log(response.data)
                           $scope.items = response.data //배열식으로 item 변수에 할당됨.
                           $location.path('/content_text')
                           //console.log($scope.items)
                       }
                       else {
                           $scope.error = response.data.message;  
                       }
                       
               }, function onError(response) {
                   $scope.error = response.data.message;
           })
*/
       }
       //togo_all 끝

 /*     
        $scope.togo_all = function(){

            //$location.path('/content_all'+$scope.BoardName)
            RequestFactory.get('/api/content/all')//+$scope.BoardName)
                .then(function onSuccess(response) {
                        if (response.status === 200) {
                            console.log('requ start, just to go')
                           // console.log(response)
                            //$scope.items = response.data
                            $location.path('/content_text')
                            //console.log(response)
  */                          
                          /*  RequestFactory.get('/api/content/manage/'+$scope.BoardName)//+$scope.BoardName)
                        
                                        .then(function onSuccess(response) {
                                            if (response.status === 200) {
                                                //RequestFactory.get('/api/content/manage'+$scope.BoardName)//+$scope.BoardName)
                                                console.log("서버간 url 공유됨!")
                                                
                                                console.log(response)
                                                //var BNsel=$scope.BoardName
                                                $scope.items = response.data
                                                //console.log("item")
                                                console.log($scope.items)
                                                console.log($scope.items.Main)
                                                $location.path('/content_text')
                                                //$location.path('/content_text')
                                            }
                                            else {
                                                $scope.error = response.data.message;  
                                            }
                                            
                                    }, function onError(response) {
                                        $scope.error = response.data.message;
                                })
                                 */
                          /*  
                            $scope.differ = $scope.BoardName
                            $location.path('/content_text')
                          */
/*                      
                        }
                        else {
                            $scope.error = response.data.message;  
                        }
                        
                }, function onError(response) {
                    $scope.error = response.data.message;
            })
            
        }
*/
        //togo_all 끝

    //init 함수
        //서버 controller에서 new data 가져오기.
        $scope.init_Main2 = function () {
            console.log("init_Main2") //$scope.delName $scope.BoardName = name
            
            RequestFactory.get('/api/content/manage/'+ $scope.BoardName)
                
                .then(function onSuccess(response) {
                        if (response.status === 200) {
                            console.log($scope.BoardName)
                            console.log('저놈')
                            console.log(response.data)
                            $scope.items = response.data //배열식으로 item 변수에 할당됨.
                            //console.log($scope.items)
                        }
                        else {
                            $scope.error = response.data.message;  
                        }
                        
                }, function onError(response) {
                    $scope.error = response.data.message;
            })
        }
    //init 끝


        if($window.sessionStorage.user !== undefined ){
            $scope.isLogin = true
        }
        //error 출력 >login.html 에서만 39-43줄 
        //$scope.error = response.data.message;
        $scope.error = '';

        //위치 변수 정의
        $scope.location = '';
        
        //del pk 정의
        $scope.delName = '';

        //update pk 정의
        $scope.upItem = '';   
        


        //<삭제 이분화>
        //삭제 클릭 시
        $scope.deleteBoard = function (name) {
            $scope.delName = name  
        }
        //삭제 > 확인 까지  클릭 시
        $scope.actionDeleteBoard = function () {

            RequestFactory.delete('/api/board/'+$scope.delName)//따로 받을게 그릇이 없어서 > url로 data를 넘기겠다.
                .then(function onSuccess(response) {
                        if (response.status === 200) {
                            console.log(response)
                            $scope.init() //리셋된 정보로 다시 화면 호출.
                        }
                        else {
                            $scope.error = response.data.message;  
                        }
                        
                }, function onError(response) {
                    $scope.error = response.data.message;
            })

        
        }


        $scope.updateBoard = function (item, isYN) {
            $scope.upItem = item  
            $scope.location = $scope.upItem.Location
            $scope.upItem.IsApproval = angular.copy(isYN)

            console.log($scope.location)
            console.log($scope.upItem)
        }

        //승인,수정 확인 누르면
        $scope.actionUpdateBoard = function () {
 
            let data = {
                Name : $scope.upItem.Name,
                Location : $scope.location, //js BoardController의 input ng-model에서 받아옴.
                IsApproval: $scope.upItem.IsApproval
            }

            RequestFactory.put('/api/board',data)
                .then(function onSuccess(response) {
                        if (response.status === 200) {
                            console.log(response)
                            $scope.init() //리로딩..
                            $scope.location = ''
                        }
                        else {
                            $scope.error = response.data.message;  
                        }
                        
                }, function onError(response) {
                    $scope.error = response.data.message;
            })

        }


        //전체 승인
        $scope.actionAllApprove = function () {
            let data = {  
                IsApprove : 1
            }
            console.log("action")
            RequestFactory.put('/api/board',data)
                .then(function onSuccess(response) {
                        if (response.status === 200) {
                            console.log(response)
                            $scope.init() //리로딩..
                            //$scope.location = ''
                        }
                        else {
                            $scope.error = response.data.message;  
                        }
                        
                }, function onError(response) {
                    $scope.error = response.data.message;
            })

        }

    


        //init 함수
        //서버 controller에서 data 가져오기.
        $scope.init = function () {
            console.log("init")
            RequestFactory.get('/api/board')
                .then(function onSuccess(response) {
                        if (response.status === 200) {
                            console.log(response.data)
                            $scope.items = response.data //배열식으로 item 변수에 할당됨.
                            console.log($scope.items)
                        }
                        else {
                            $scope.error = response.data.message;  
                        }
                        
                }, function onError(response) {
                    $scope.error = response.data.message;
            })
        }

    }
]);
