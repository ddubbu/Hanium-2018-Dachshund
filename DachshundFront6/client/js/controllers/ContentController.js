...
///
     ...
     */
            ...
            $scope.changeCheckBox = function (No) {
                ...
                $scope.setAllChecked = function (sel) {
            ...
            $scope.init_Content = function () {
                ...
                console.log('파람스로 url',$scope.BoardName );
                        ...
                        RequestFactory.delete('/api/content/manage/' + $scope.items[i].No)
                            .then(function onSuccess(response) {
                                if (response.status === 200) {
                                    ...
                                    //$scope.items = response.data //배열식으로 item 변수에 할당됨.
                                    // $scope.setAllChecked(true);
                                    $scope.init_Content()  //리셋된 정보로 다시 화면 호출.
                                }
                                else {
                                    $scope.error = response.data.message;
                                }
            ...
            $scope.togoModify = function () {
                let data = {
                    //store.XX가 자동으로 ng-model 기능을 잡음.
                    //그러므로 inputData-> store.XX로 나중에 바꾸자
                    no: $scope.inputData.No,
                    b_sel: $scope.inputData.B_sel,
                    t_sel: $scope.inputData.T_sel,
                    s_date: $scope.inputData.start_date,
                ...
                //console.log('ng-model inputData 변수로 저장')
                //console.log(data)
                console.log('RequestFactory.put 바로 직전')
            ...
            RequestFactory.put('/api/content/modify/' + $routeParams.Num,data)//+ <= 수정할 items No 넘기기$scope.items[i].No)
                .then(function onSuccess(response) {
                    if (response.status === 200) {
                        ...
                        console.log($routeParams.Num)
                        console.log('수정을 위한 Num 잘 넘김')
                        //수정 잘했으면 관리자 창으로 넘어가기
            ...
            $scope.init_Modify = function () {
                ...
                RequestFactory.get('/api/content/modify/' + N)
                    .then(function onSuccess(response) {
                        if (response.status === 200) {
                            //console.log(response.data)                           
                            $scope.store = response.data[0] //배열[0]식으로 item 변수에 할당됨.
                            
                            //받아온 데이터를 미리 받아놓자.
                            //굳이 안받고 store로 계속 이용하려 했는데 따로 하자.
                            //$scope.inputData.No=$scope.store.no
                            ...
                            console.log('init_Modify 잘 하고 있는지 store 출력')
                            console.log($scope.store)
                            console.log('inputData<-store')
                ...
                .then(function onSuccess(response) {
                    ...
                    // console.log(response.data)
                        $scope.items = response.data //배열식으로 item 변수에 할당됨.
                        console.log('RequestFactory 들어옴')
                        console.log($scope.items)
                        //console.log($scope.inputData.B_sel)
